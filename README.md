...
GScomp-QA captures a wide range of visual artifacts arising from both the view synthesis process and GS compression. Understanding these artifacts is essential for designing novel compression solutions and quality metrics. The main artifact types observed in the dataset are described below.

<p align="center"><strong>Geometry Distortion</strong></p> <p align="center"> <img src="imgs/geom_comp.png" width="45%" /> <img src="imgs/geom_ref.svg" width="45%" /> </p>

Geometry distortions correspond to structural inaccuracies in the reconstructed scene, such as deformed surfaces or misaligned objects. These artifacts originate both from the GS synthesis process and from compression.

Generated by both synthesis and compression
Present in both baseline models and compression solutions
...
Floaters are spurious Gaussian primitives that appear as small, disconnected elements floating in space. They are inherent to the GS representation and often persist after compression.
...
Color shifting refers to deviations in color reproduction, such as hue or intensity changes. These artifacts are introduced by compression, particularly when Gaussian parameters are transformed into image-like representations.

Compression-generated
Common in color-oriented compression methods (e.g., GSICO-3DGS, Compact3D, SOG)
...
Blurring manifests as a loss of fine details and texture sharpness. It typically results from strong regularization or quantization during compression-aware training.

Compression-generated
Common in joint training and compression methods (e.g., ContextGS, HAC++, Reduced3DGS)
<p align="center"><strong>Temporal Inconsistencies</strong></p>

Temporal inconsistencies include flickering, unstable geometry, or view-dependent variations that become noticeable during camera motion. These artifacts are difficult to capture in static images.

Synthesis-generated
Present in both baseline models and post-training compression solutions
<p align="center"><strong>Summary</strong></p>

The coexistence of synthesis- and compression-induced artifacts highlights the complexity of evaluating GS compression. GScomp-QA provides a controlled setting to analyze these distortions and supports the development of metrics capable of capturing their perceptual impact.